...
from net.Backbone_3D.model_2D.models.backbones.sr_backbones.basicvsr_net import (
    ResidualBlocksWithInputConv, SPyNet)
from net.Backbone_3D.model_2D.models.common import PixelShufflePack, flow_warp
...
class DPATISR(nn.Module):
    ...
    def __init__(self,
        ...
        # modified to num_classes
        if bayesian:
            self.conv_last = nn.Conv2d(64, 2*self.num_classes, 3, 1, 1)
        else:
            self.conv_last = nn.Conv2d(64, self.num_classes, 3, 1, 1)
        '''if bayesian:
    ...
    def forward(self, lqs):
        ...
        '''print(flows_forward.size(), flows_backward.size())
        import matplotlib.pyplot as plt
        plt.figure()
        plt.subplot(131)
        plt.imshow(self.to_cpu(lqs[0,0,0,:,:]))
        plt.subplot(132)
        plt.imshow(self.to_cpu(flows_forward[0,0,0,:,:]))
        plt.subplot(133)
        plt.imshow(self.to_cpu(flows_backward[0,0,0,:,:]))
        plt.show()'''
...
if __name__ == "__main__":
    device = 'cuda'
    model = DPATISR(num_classes=2).to(device)
    while 1:
        Input = torch.ones((1, 4, 1, 128, 128), device=device)
        Output = model(Input)
        print(Output.size())
