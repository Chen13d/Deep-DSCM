...
class SpatialSpectralSRNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=31, n_channels=64, n_blocks=7, kernel_size=3, upscale_factor=4):
        ...
        if upscale_factor == 1:
            self.post_spatial_layers = nn.Sequential(
                nn.Conv2d(in_channels=n_channels, out_channels=n_channels//4, kernel_size=kernel_size, padding=kernel_size // 2, bias=True),
        ...
        if upscale_factor == 1:
            ...
            nn.Conv2d(in_channels=5 * n_channels, out_channels=n_channels, kernel_size=kernel_size, padding=kernel_size // 2, bias=True),
            nn.ReLU(inplace=True),
            #nn.PixelShuffle(upscale_factor),
            nn.Conv2d(in_channels=n_channels, out_channels=n_channels, kernel_size=kernel_size, padding=kernel_size // 2, bias=True)
            )
            ...
            self.pre_fusion_layers = nn.Sequential(
                ...
                nn.Conv2d(in_channels=5 * n_channels, out_channels=16 * n_channels, kernel_size=kernel_size, padding=kernel_size // 2, bias=True),
                nn.ReLU(inplace=True),
                nn.PixelShuffle(upscale_factor),
                nn.Conv2d(in_channels=n_channels, out_channels=n_channels, kernel_size=kernel_size, padding=kernel_size // 2, bias=True)
            )
    ...
    def forward(self, x):
        ...
        return out, out_spectral, out_spatial
...
if __name__ == "__main__":
    ...
    model = SpatialSpectralSRNet(in_channels=1, out_channels=2, upscale_factor=1).cuda()
    spatial, spectral, output = model(input)
    print(input.size(), spatial.size(), spectral.size(), output.size())