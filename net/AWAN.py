...
class NONLocalBlock2D(nn.Module):
    ...
    def forward(self, x):
        batch_size = x.size(0)
        #print(x.size(), self.in_channels, self.inter_channels)
...
class PSNL(nn.Module):
    def __init__(self, channels):
        ...
        # nonlocal module
        self.non_local = NONLocalBlock2D(channels, reduction=8)
...
class AWAN(nn.Module):
    def __init__(self, inplanes=3, planes=31, channels=200, n_DRBs=8):
        ...
        self.output_conv2D = Conv3x3(channels, 31, 3, 1)
        self.tail_nonlocal = PSNL(31)
        self.pred_layer = nn.Conv2d(31, planes, kernel_size=1, padding=0, stride=1)
    def forward(self, x):
        out = self.DRN2D(x)
        return out
...
if __name__ == "__main__":
    ...
    input_tensor = torch.rand((1, 1, 256, 256)).cuda()
    model = AWAN(inplanes=1, planes=2, channels=200, n_DRBs=10).cuda()
