...
from dataset.read_prepared_data_DSCM_FLIM import *

#options of .yml format in "options" folder
opt_path = 'options/5_train_1_stage_STED_DSCM_FLIM_simulation.yml'
...
def main():
    ...
    if opt['read_version'] == "real-time":
        ...
        #train_loader, val_loader, num_train_image = gen_prepared_dataloader_FLIM(read_dir_train=read_dir_train, read_dir_val=read_dir_val, num_file_train=opt['num_train'], 
        ...
        train_dataset = prepared_dataset_FLIM(read_dir=read_dir_train, num_file=opt['num_train'], num_org=len(opt['category']), org_list=opt['category'], size=opt['size'], device=opt['device'])
        val_dataset = prepared_dataset_FLIM(read_dir=read_dir_val, num_file=opt['num_test'], num_org=len(opt['category']), org_list=opt['category'], size=opt['size'], device=opt['device'], random_selection=False, flip_flag=False, crop_flag=False)
        train_loader = DataLoader(train_dataset, shuffle=True, batch_size=opt['train']['batch_size'], num_workers=2, persistent_workers=True)
    ...
    for epoch in range(opt['train']['epoches']):
        ...
        for iter in range(opt['train']['num_iter']):
            ...
            for batch_index, data in enumerate(train_loader):  
                Input, GT_DS, GT_D, GT_S, denoised, lifetime, sta = data
        ...
        # validat   ·e per "epoches_per_test"
...
if __name__ == "__main__":
    #main()
    if 1:
        #opt_path = 'options/5_train_1_stage_STED_DSCM_FLIM_simulation2.yml'
        #opt = parse(opt_path=os.path.join(cwd, opt_path))
        #main()
        opt_path = 'options/5_train_1_stage_STED_DSCM_FLIM_simulation3.yml'
        opt = parse(opt_path=os.path.join(cwd, opt_path))
        main()
        opt_path = 'options/5_train_1_stage_STED_DSCM_FLIM_simulation4.yml'
        opt = parse(opt_path=os.path.join(cwd, opt_path))
        main()