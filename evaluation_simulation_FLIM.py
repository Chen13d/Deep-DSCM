...
opt_path = 'options/Simulation_eval_FLIM.yml'
...
def main():
    ...
    if opt['read_version'] == "real-time":
        ...
        train_dataset = prepared_dataset_FLIM(read_dir=read_dir_train, num_file=opt['num_train'], num_org=len(opt['category']), org_list=opt['category'], size=opt['size'], device=opt['device'], crop_flag=False, flip_flag=False)
        val_dataset = prepared_dataset_FLIM(read_dir=read_dir_val, num_file=opt['num_test'], num_org=len(opt['category']), org_list=opt['category'], size=opt['size'], device=opt['device'], random_selection=False, crop_flag=False, flip_flag=False)
    ...
    df.to_excel(os.path.join(excel_path, f"{opt['degeneration_w0']}_{opt['noise_level']}_FLIM.xlsx"))
