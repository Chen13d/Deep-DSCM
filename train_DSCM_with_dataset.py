...
opt_path = 'options/train_with_dataset.yml'
...
def main():
    ...
    net_main = DSCM_with_dataset(opt, in_channels=1, num_classes=len(opt['category']), model_name_G=opt['net_G']['model_decouple_name'], 
                          ...
                          scheduler_name=opt['train']['scheduler'], device=device, weight_list=opt['net_G']['weight_decouple'], 
    ...
    if opt['read_version'] == "real-time":
        ...
        if not opt['FLIM']:
            ...
            val_dataset = prepared_dataset_FLIM(read_dir=read_dir_val, num_file=opt['num_file_val'], num_org=len(opt['category']), org_list=opt['category'], size=opt['size'], device=opt['device'], 
                                                random_selection=False, crop_flag=False, flip_flag=False)
        if opt['num_workers']:
            train_loader = DataLoader(train_dataset, shuffle=True, batch_size=opt['train']['batch_size'], num_workers=opt['num_workers'], persistent_workers=True)
        else:
    ...
    for epoch in range(opt['train']['epoches']):
        ...
        for iter in range(opt['train']['num_iter']):
            ...
            for batch_index, data in enumerate(train_loader):  
                ...
                if opt['FLIM']: 
                    fake_main = net_main.feed_data(Input=torch.concatenate([Input, sorted_image], dim=1), GT=GT_DS)
                else:
            ...
            with torch.no_grad():
                ...
                for batch_index, data in enumerate(val_loader):
                    ...
                    if (epoch+1) % opt['train']['epoches_per_save'] == 0:
                        ...
                        sorted_list.append(sorted_image)
                        sta_list.append(sta)
                    for col in range(7):
                        curr_list[col].append(loss_main[col].item()) if loss_main[col] != 0 else curr_list[col].append(0)
                    # PCC loss
                ...
                if (epoch+1) % opt['train']['epoches_per_save'] == 0:
                    if opt['FLIM']:
                        toolbox.gen_validation_images_FLIM(data_list=[Input_list, GT_list, sorted_list, sta_list])
                    else:
...
if __name__ == "__main__":
    ...
    if 0:
        #options of .yml format in "options" folder
        opt_path = 'options/train_with_dataset2.yml'
        # read options
        opt = parse(opt_path=os.path.join(cwd, opt_path))
        main()
    ...
    if 0:
        #options of .yml format in "options" folder
        opt_path = 'options/train_with_dataset3.yml'
        # read options
        opt = parse(opt_path=os.path.join(cwd, opt_path))
        main()