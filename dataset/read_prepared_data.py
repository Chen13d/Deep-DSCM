...
def rand_crop_pair(Input, GT, denoised, size):
...
class prepared_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(GT_DS.size()) == 2: GT_DS = GT_DS.unsqueeze(0)
        # crop or not
        if self.crop_flag:
            Input, GT_DS, denoised = rand_crop_pair(Input=Input, GT=GT_DS, denoised=denoised, size=self.size)
        else:
        ...
        return Input, GT_DS, 0, 0, 0, denoised, statistic_dict
    

def gen_prepared_dataloader(read_dir_train, read_dir_val, num_file_train, num_file_val, num_org, org_list, size, batch_size, device):
    train_dataset = prepared_dataset(read_dir=read_dir_train, num_file=num_file_train, num_org=num_org, org_list=org_list, size=size, device=device)
    val_dataset = prepared_dataset(read_dir=read_dir_val, num_file=num_file_val, num_org=num_org, org_list=org_list, size=size, device=device)
    train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=batch_size, num_workers=16, pin_memory=True, persistent_workers=True)
